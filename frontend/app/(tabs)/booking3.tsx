...
// ─────────────────────────────────────────────
// 📘 LESSON: Replacing ALL Alert.alert() calls
// Alert.alert() is a native OS dialog — invisible on web.
// The fix: build every feedback UI (success, error, validation)
// as React Native components that render on all platforms.
// ─────────────────────────────────────────────

// ── Reusable Overlay ─────────────────────────
// One component handles BOTH success and error states.
// The 'type' prop switches between the two visuals.
    ...
    <Animated.View style={[styles.overlayBackdrop, { opacity: backdropAnim }]}>
      ...
      >
        ...
        >
          ...
          {/* Success-only: booking details grid */}
          ...
          {/* Secondary button — only shown when provided (e.g. "Try Again") */}
...
// ─────────────────────────────────────────────
// Inline validation banner — replaces Alert for
// simple field-level errors (missing info, bad dates)
// ─────────────────────────────────────────────
...
const BookingScreen = () => {
  ...
  // ── Feedback states ──────────────────────────
  // Instead of Alert.alert(), we use these three states
  // to show inline validation, success overlay, or error overlay
  ...
  const handleBooking = async () => {
    // ── Inline validation — no Alert needed ──
    ...
    if (nights <= 0) {
      setValidationMsg("Check-out date must be after check-in date.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        // Try to read a specific error from the backend
      ...
      // ✅ Error — show error overlay with the specific message
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Inline validation banner — appears instead of Alert */}
        {validationMsg && (
          <ValidationBanner
      ...
      {overlayState === "error" && (
        <FeedbackOverlay
          ...
          onPrimary={() => setOverlayState(null)} // dismiss → user can fix and retry
...
const styles = StyleSheet.create({
  ...
  // Inline validation banner — slides in above form
  validationBanner: {
